<template>
<div class="content-wrap-mini" style="width: 700px; margin: 20px auto;">
			...
			tinymce.init({
					...
					setup: (editor) => {
						...
						setTimeout(() => {
							...
							//回填邮件内容
							editor.setContent(this.mailDetail.content)
						},500);
					},
			})
		},10);		
...
mounted() {
  ...
  methods: {
	  	saveA(){
			this.EditorContentA = ''
			//换图片标签内容
			this.EditorContentA  = tinymce.editors[this.emailIndex].getContent()
		...
		saveB(){
			this.EditorContentB  = tinymce.editors[this.emailIndex].getContent()
